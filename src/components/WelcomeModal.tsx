...
const APPS: { icon: string; name: string; blurb: string }[] = [
  { icon: '✨', name: 'AI Assistant', blurb: 'ask about my work in plain English' },
  {
    icon: '🗺️',
    name: 'Workspace Map',
    blurb: 'see how the projects, skills and experience connect',
  },
  { icon: '🗂️', name: 'Projects', blurb: 'the case files, with the tech behind each' },
  {
    icon: '⌨️',
    name: 'Terminal',
    blurb: 'where most of the depth lives - dozens of commands, a few secrets, even a game. Type help to start.',
  },
]
      ...
      {open && power === 'running' && (
        ...
        >
          <motion.div
            ...
            className="max-h-[85vh] w-full max-w-[400px] overflow-y-auto rounded-2xl border border-desk-edge bg-desk-panel p-6 shadow-2xl"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-lg font-semibold text-desk-text">
              ...
              <button
                onClick={close}
                aria-label="Close"
                className="-mr-1 -mt-1 shrink-0 rounded p-1 text-desk-muted hover:text-desk-text"
              >
                &times;
              </button>
            </div>

            <p className="mt-3 text-sm text-desk-text">
            ...
            <ul className="mt-4 space-y-2.5">
              {APPS.map((a) => (
                <li key={a.name} className="flex gap-2.5 text-sm">
                  <span className="shrink-0 text-[17px] leading-tight">
                    {a.icon}
                  </span>
                  <span className="text-desk-muted">
                    <span className="font-medium text-desk-text">{a.name}</span>
                    {' - '}
                    {a.blurb}
                  </span>
                </li>
            ...
            <p className="mt-4 rounded-lg border border-desk-edge bg-desk-bg/60 px-3 py-2 text-xs text-desk-muted">
              {mobile
            ...
            <div className="mt-5 flex items-center justify-between">
              <span className="text-[10px] uppercase tracking-widest text-desk-muted">
